...
function onSelect(message, flags) {
    var self = this, valueKey = this.opts.vk, name = message["3" /* name */], value = valueKey ? message[valueKey] : (message["2" /* id */] || message["1" /* value */]);
    if (!flags) {
    ...
    else if ((this.flags & 32 /* CBFN_AFTER_SET */) && this.cbfn) {
        self.pending_name = null;
...
function postPS(req, opts) {
    return rpc.post(this, JSON.stringify(req));
}
export function parseOpts(args, value, vm, el) {
    var i = 0, len = !args ? 0 : args.length, flags = i === len ? 0 : parseInt(args[i++], 10), pojo = value.pojo, field = value.field, fetch = value.fetch, cbfn = value.onSelect, pojo_ = pojo._, fk = field;
    if (!pojo_) {
        pojo_ = pojo;
        fk += '$';
    }
...
function focusout(e) {
    ...
    if (name) {
        ...
        else if ((self.flags & 32 /* CBFN_AFTER_SET */) && self.cbfn) {
            self.pojo_[self.fk] = name;
        ...
        self.pojo[self.field] = null;
        0 !== (self.flags & 64 /* CBFN_ON_UNSET */) && self.cbfn(self.field, null, null);
    }
...
function keydown(e) {
    ...
    switch (e.which) {
        ...
        case 13 /* ENTER */:
            ...
            else if ((self.flags & 32 /* CBFN_AFTER_SET */)) {
