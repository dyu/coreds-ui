...
function onSelect(message, flags) {
    var self = this, name = message["1" /* name */], value = message["3" /* id */] || message["2" /* value */];
    if (!flags) {
...
export function parseOpts(args, pojo, field, fetch, cbfn, vm, el) {
    var i = 0, len = !args ? 0 : args.length, flags = i === len ? 0 : parseInt(args[i++], 10), pojo_ = pojo._, fk = field;
    if (!pojo_) {
        pojo_ = pojo;
        fk += '$';
    }
