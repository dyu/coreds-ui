...
function selectDayNT() {
    var config = this.config, current_entry = config.current_entry, 
...
var Calendar = (function () {
    ...
    Calendar.created = function (self) {
        instance = self;
        var date = new Date(), year = date.getFullYear(), month = date.getMonth(), day = date.getDate(), today = new Date(year, month, day), todayUTC = localToUtc(today.getTime()), startDate = { year: year, month: month, day: day }, opts = { weekStart: 0 }, cache = {}, current_entry = getEntry(year, month, opts, startDate, cache);
        defp(self, 'config', {
            today: today,
            todayUTC: todayUTC,
            startDate: startDate,
        ...
        var pstore = defp(self, 'pstore', new PojoStore(current_entry.array, {
            desc: true,
            pageSize: 42,
...
export { Calendar };
var item_tpl = "\n<li :class=\"'type-' + pojo.flags + ((pojo._.lstate & " + 2 /* SELECTED */ + ") ? ' day active' : ' day')\"\n    v-show=\"pojo.day\" v-text=\"pojo.day\"></li>\n"; /**/
