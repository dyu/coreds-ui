...
function itemLookupTC(target, p) {
    try {
...
function trigger(self, el, ev) {
    var focusValue = el.dataset.focus, focusFn;
    fireEvent(el, ev);
    if (!focusValue || null === (focusFn = el['d_focus']))
        return;
...
function select(self, e, dbltap, clickedUpdate) {
    var target = e.target, pojo, pager, store, pressValue;
...
keymage.$('pager', 'defmod-shift-space', function (e) { var pojo; if (current && (pojo = current.pager.pojo) && pojo['1'])
    current.pager['store'].select(pojo, 4 /* CLICKED_UPDATE */); });
